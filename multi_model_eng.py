...
# We're refactoring to use LiteLLM for model calls instead of Ollama
...
# Models
MAIN_MODEL = os.getenv("MAIN_MODEL", "anthropic/claude-3-sonnet-20240620")
TOOLCHECKER_MODEL = os.getenv("TOOLCHECKER_MODEL", "groq/llama3-70b-8192-tool-use-preview")
CODEEDITOR_MODEL = os.getenv("CODEEDITOR_MODEL", "mistral/codestral")
CODEEXECUTION_MODEL = os.getenv("CODEEXECUTION_MODEL", "openai/gpt-4o-mini")

# Token usage tracking
token_usage = {model: {"input": 0, "output": 0} for model in [MAIN_MODEL, TOOLCHECKER_MODEL, CODEEDITOR_MODEL, CODEEXECUTION_MODEL]}

def track_token_usage(kwargs, completion_response, start_time, end_time):
    model = kwargs['model']
    token_usage[model]['input'] += completion_response['usage']['prompt_tokens']
    token_usage[model]['output'] += completion_response['usage']['completion_tokens']
    print(f"Token usage for {model}: Input: {token_usage[model]['input']}, Output: {token_usage[model]['output']}")

# Set up LiteLLM callback
completion.success_callback = [track_token_usage]
